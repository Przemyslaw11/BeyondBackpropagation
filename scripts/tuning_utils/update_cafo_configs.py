...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Update a Cascaded Forward (CaFo) YAML config file with the best hyperparameters from an Optuna study.",
...
def update_cafo_config_from_optuna(
    ...
    try:
        ...
        elif study_name not in [s.study_name for s in loaded_studies]: logger.error(f"Specified study name '{study_name}' not found in {db_path}"); return False
    ...
    try:
        ...
        if not best_params: logger.warning("Best trial has no parameters. Cannot update config."); return True
    ...
    for optuna_key, value in best_params.items():
        ...
        if optuna_key == "pred_lr": yaml_key = "predictor_lr"
        elif optuna_key == "pred_wd": yaml_key = "predictor_weight_decay"
        elif optuna_key == "epochs_per_block": yaml_key = "num_epochs_per_block"
        elif optuna_key == "block_lr": yaml_key = "block_lr" # If tuning block params
        elif optuna_key == "block_wd": yaml_key = "block_weight_decay" # If tuning block params
        elif optuna_key == "block_epochs": yaml_key = "block_training_epochs" # If tuning block epochs
    ...
    if not updated_values: logger.warning("No relevant CaFo parameters found in best trial to update."); return True
