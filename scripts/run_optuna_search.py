...
def parse_args():
    ...
    parser.add_argument("--n-trials",type=int,default=None,help="Number of trials to run (overrides config).")
...
def main():
    ...
    try:
        ...
        logger.info("Initial Config for Optuna:"); config_str = pprint.pformat(config)
    ...
    sampler_type = tuning_config.get("sampler", "TPE").upper(); pruner_type = tuning_config.get("pruner", "Median").upper()
    if algorithm_name in ["MF", "CAFO", "FF"] and pruner_type != "NONE": logger.warning(f"{algorithm_name} Optuna objective reports only final metric. Pruner '{pruner_type}' might not be effective. Consider 'NONE'.")
    optuna_seed = config.get("general", {}).get("seed", 42)
    sampler_map = {"TPE": optuna.samplers.TPESampler(seed=optuna_seed),"RANDOM": optuna.samplers.RandomSampler(seed=optuna_seed),}
    pruner_map = {"MEDIAN": optuna.pruners.MedianPruner(),"HYPERBAND": optuna.pruners.HyperbandPruner(),"NONE": optuna.pruners.NopPruner(),}
    sampler = sampler_map.get(sampler_type, optuna.samplers.TPESampler(seed=optuna_seed)); pruner = pruner_map.get(pruner_type, optuna.pruners.MedianPruner())
    if sampler_type not in sampler_map: logger.warning(f"Unsupported sampler '{sampler_type}', using TPE.")
    if pruner_type not in pruner_map: logger.warning(f"Unsupported pruner '{pruner_type}', using Median.")
    study_direction = tuning_config.get("direction", "maximize")
    logger.info(f"Using sampler: {sampler_type}, pruner: {pruner_type}"); logger.info(f"Optimization direction: {study_direction}"); logger.info(f"Database storage: {storage_path}")

    try:
        study = optuna.create_study(study_name=study_name, storage=storage_path, direction=study_direction, sampler=sampler, pruner=pruner, load_if_exists=True)
        ...
        if algorithm_name == "BP":
            best_config_update = {"optimizer": {}}; update_section_name = "optimizer"
            for key, value in params_to_log.items(): logger.info(f"    {key}: {value}"); best_config_update["optimizer"]["weight_decay" if key=="wd" else key] = value
            ...
            for key, value in params_to_log.items(): logger.info(f"    {key}: {value}"); best_config_update["algorithm_params"]["weight_decay" if key=="wd" else key] = value
            ...
            for key, value in params_to_log.items():
                ...
                if key == "pred_lr": best_config_update["algorithm_params"]["predictor_lr"] = value
                elif key == "pred_wd": best_config_update["algorithm_params"]["predictor_weight_decay"] = value
                elif key == "epochs_per_block": best_config_update["algorithm_params"]["num_epochs_per_block"] = value
                elif key == "block_lr": best_config_update["algorithm_params"]["block_lr"] = value
                elif key == "block_wd": best_config_update["algorithm_params"]["block_weight_decay"] = value
            ...
            for key, value in params_to_log.items():
                logger.info(f"    {key}: {value}")
                if key == "ff_lr": best_config_update["algorithm_params"]["ff_learning_rate"] = value
        ...
        study_results_summary = {
            ...
            f"best_{metric_name}": best_trial.value, "best_hyperparameters_raw": best_trial.params,
            "config_override": best_config_update,
        }
        try:
            with open(best_params_file, "w") as f: yaml.dump(study_results_summary, f, default_flow_style=False, sort_keys=False)
            logger.info(f"Best parameters saved to: {best_params_file}"); logger.warning("=" * 50); logger.warning(f"REMINDER: Manually update '{update_section_name}' in '{args.config}' or check automatic update script."); logger.warning("=" * 50)
        except Exception as e: logger.error(f"Failed to save best parameters: {e}")

    except ImportError: logger.error("optuna library not found.")
    except Exception as e: logger.error(f"An error occurred during the Optuna study: {e}", exc_info=True)
