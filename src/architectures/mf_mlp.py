...
class MF_MLP(nn.Module):
    ...
    def __init__(
        ...
        # Final classifier layer (W_L) - used for BP-style prediction and BP baseline
        ...
        # Index 'i' corresponds to the activation a_i.
        # M_0 projects a_0 (input), M_1 projects a_1 (output of hidden 0), etc.
        # We need M_0 to M_{L-1}. L = num_hidden_layers.
        self.projection_matrices = nn.ParameterList()
        dims_for_M = [input_dim] + hidden_dims  # Dimensions of a_0, a_1, ..., a_{L-1}
        for i, layer_dim in enumerate(dims_for_M):
            ...
            # logger.debug(f"Created projection matrix M_{i} with shape {m_matrix.shape} (for activation a_{i})")
