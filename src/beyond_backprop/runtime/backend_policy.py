"""Canonical backend policy for local and Slurm-style execution."""
...
try:
    import torch
except ImportError:  # pragma: no cover - permits lightweight import validation.
...
def _mps_is_available() -> bool:
    ...
    return bool(
        callable(is_available) and callable(is_built) and is_available() and is_built()
    )
...
class ExecutionBackend:
    ...
    def resolve_device(self, device_preference: str = "auto") -> torch.device:
        ...
        if preference == "mps":
            return (
                torch.device("mps") if _mps_is_available() else self._fallback_device()
            )
    ...
    def resolve_dataloader_defaults(
        self, config: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """Return backend defaults with explicit config precedence."""
        if torch is None:
    ...
    def resolve_results_dir(self, config: dict[str, Any]) -> str:
        ...
        backend_config = config.get("backend", {}).get(self.name, {})
        explicit_dir = backend_config.get("results_dir") or config.get(
            "results", {}
        ).get("dir")
        if explicit_dir:
            return explicit_dir
