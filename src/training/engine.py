...
def get_model_and_adapter(
    ...
    This factory function reads the configuration to determine which model
    architecture to build (e.g., FF_MLP, MF_MLP, CaFo_CNN). It handles the
    special cases where a baseline Backpropagation (BP) model needs to be
    constructed from the specifications of an alternative architecture.
    It also creates an appropriate input adapter (e.g., for flattening
    images) if the architecture requires it.
    ...
    Returns:
        A tuple containing:
        - The instantiated `nn.Module` model.
        - An optional callable function that preprocesses the input tensor
    ...
    logger.debug(
        "get_model_and_adapter: Raw arch name='%s', Lowercase='%s'",
        arch_name_raw,
        arch_name,
    )
    ...
    if arch_name in ["ff_mlp", "mf_mlp"]:
        if "input_dim" not in arch_params:
            ...
            logger.debug(
                f"Calculated input_dim={arch_params['input_dim']} for {arch_name}"
            )
        ...
        raise ValueError(
            f"Unknown model architecture category for name: {arch_name}. "
            "Expected 'ff_mlp', 'mf_mlp', or 'cafo_cnn'."
        )
    ...
    if arch_name in ["ff_mlp", "mf_mlp"]:
        ...
        if arch_name == "ff_mlp":
            if is_bp_baseline:
                logger.info(
                    "Adapting modified FF_MLP structure for BP baseline -> "
                    "Standard nn.Sequential MLP."
                )
                ...
                logger.debug(
                    "Created BP baseline Sequential model from modified FF_MLP spec."
                )
            else:
        ...
        if is_bp_baseline:
            logger.info(
                f"Creating BP baseline model from {arch_name.upper()} blocks + "
                "final Linear layer."
            )
            cnn_base.to(device)
            with torch.no_grad():
                ...
                if hasattr(cnn_base, "final_flat_dim"):
                    ...
                    logger.debug(
                        f"Got flattened output dimension from {arch_name.upper()} "
                        f"attribute: {num_output_features}"
                    )
                else:
                    ...
                    num_output_features = dummy_features.view(
                        dummy_features.size(0), -1
                    ).shape[1]
                    logger.debug(
                        "Dynamically computed flattened output dimension from "
                        f"{arch_name.upper()} blocks: {num_output_features}"
                    )
            cnn_base.cpu()
            ...
            logger.debug(
                f"Created BP baseline Sequential model from {arch_name.upper()} spec."
            )
        else:
            model = cnn_base
        ...
        logger.error(
            "Architecture check failed inside factory. Lowercase name '%s' "
            "did not match ['ff_mlp', 'mf_mlp'] or ['cafo_cnn']. Check logic.",
            arch_name,
        )
        raise ValueError(
            f"Unknown model architecture: {arch_name}. Expected 'ff_mlp', "
            "'mf_mlp', or 'cafo_cnn'."
        )
...
def run_training(
    ...
    This function is the main entry point for running an experiment. It handles:
    - Setting up seeds, devices, logging (W&B), and monitoring (NVML, CodeCarbon).
    - Loading data and creating DataLoaders.
    - Instantiating the model and its adapter.
    - Profiling the model for FLOPs.
    - Calling the appropriate algorithm-specific training function.
    - Calling the evaluation function on the test set.
    - Aggregating and logging all results and metrics.
    - Cleaning up resources.

    Args:
        config: The main experiment configuration dictionary.
        wandb_run: An optional active W&B run object. If None, a new one
            will be initialized.

    Returns:
        A dictionary containing the experiment results, including metrics,
        durations, and resource consumption measurements.
    """
    results = {}
    nvml_active = False
    gpu_handle = None
    ...
    try:
        ...
        if env_seed_str is not None:
            try:
                ...
                logger.info(
                    f"Found EXPERIMENT_SEED environment variable: {env_seed}. "
                    f"Overriding config seed ({seed})."
                )
                seed = env_seed
                general_config["seed"] = seed
            except ValueError:
                logger.warning(
                    f"EXPERIMENT_SEED environment variable ('{env_seed_str}') is not "
                    f"a valid integer. Using config seed ({seed})."
                )
            ...
            if init_nvml():
                ...
                if gpu_handle:
                    ...
                    if mem_info_start:
                        ...
                        logger.info(
                            f"Initial GPU Mem: {mem_info_start[0]:.2f} MiB Used / "
                            f"{mem_info_start[1]:.2f} MiB Total"
                        )
                else:
                    logger.warning(
                        f"NVML active but failed get handle for GPU {gpu_index}."
                    )
                    nvml_active = False
                if monitoring_config.get("energy_enabled", True) and gpu_handle:
                    ...
                    logger.info(
                        "GPU Energy monitor initialized (Interval: "
                        f"{monitor._interval_sec}s)."
                    )
                elif monitoring_config.get("energy_enabled", True):
                    logger.warning(
                        "Energy monitoring enabled but failed to get GPU handle."
                    )
                else:
        ...
        initial_metrics["codecarbon_country_iso"] = results.get(
            "codecarbon_country_iso", "N/A"
        )
        ...
        if profiling_config.get("enabled", True):
            try:
                ...
                profile_input_constructor = (
                    (lambda: input_adapter(sample_input_device[:1]))
                    if input_adapter
                    else (lambda: sample_input_device[:1])
                )
                ...
                if fwd_gflops is not None:
                    results["estimated_fwd_gflops"] = fwd_gflops
                    initial_metrics["estimated_fwd_gflops"] = fwd_gflops
                    logger.info(f"Estimated Forward Pass GFLOPs: {fwd_gflops:.4f} G")
                    if algorithm_name == "bp":
                        estimated_bp_update_gflops = fwd_gflops * 3.0
                        results["estimated_bp_update_gflops"] = (
                            estimated_bp_update_gflops
                        )
                        initial_metrics["estimated_bp_update_gflops"] = (
                            estimated_bp_update_gflops
                        )
                        logger.info(
                            "Estimated BP Update Cycle GFLOPs (Fwd+Bwd ~3x Fwd): "
                            f"{estimated_bp_update_gflops:.4f} G"
                        )
                    else:
                        results["estimated_bp_update_gflops"] = float("nan")
                        initial_metrics["estimated_bp_update_gflops"] = float("nan")
                        logger.info(
                            f"Algorithm '{algorithm_name}' is BP-free, Est. BP "
                            "Update GFLOPs is N/A."
                        )
                else:
        ...
        if initial_metrics:
            ...
            logger.debug(
                f"Logging initial metrics at global_step {step_ref[0]}: "
                f"{initial_metrics.keys()}"
            )
        ...
        with monitor if monitor else contextlib.nullcontext():
            ...
            if isinstance(train_output, (float, int)):
                ...
                logger.info(
                    "Received Peak GPU Memory (sampled during training): "
                    f"{peak_gpu_mem_during_training:.2f} MiB"
                )
            else:
                logger.warning(
                    f"Training function for '{algorithm_name}' returned type "
                    f"{type(train_output)}. Expected peak memory (float/int). "
                    "Peak memory set to NaN."
                )
                peak_gpu_mem_during_training = float("nan")
        ...
        if algorithm_name in ["bp", "cafo", "mf"]:
        ...
        try:
            ...
            if isinstance(eval_output, dict):
                ...
                eval_results[test_acc_key] = eval_output.get(
                ...
                logger.error(
                    f"Unexpected eval return type: {type(eval_output)}. "
                    "Setting results to NaN."
                )
                eval_results = {
                    test_loss_key: float("nan"),
                    test_acc_key: float("nan"),
                }
        except Exception as e:
        ...
        logger.info(
            f"Test Set Results: Acc: {eval_results.get(test_acc_key, 'N/A'):.2f}%, "
            f"Loss: {eval_results.get(test_loss_key, 'N/A'):.4f}"
        )
        ...
        codecarbon_emissions_kg = float("nan")
        codecarbon_emissions_g = float("nan")
        ...
        if tracker:
            ...
            try:
                ...
                if emissions_data is not None:
                    logger.info(
                        "CodeCarbon tracker stopped. Attempting to read from: "
                        f"{carbon_csv_path}"
                    )
                else:
                    logger.warning(
                        "CodeCarbon tracker.stop() returned None. "
                        "Attempting CSV read."
                    )
                ...
                if carbon_csv_path and os.path.exists(carbon_csv_path):
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            if not df.empty and "emissions" in df.columns:
                                ...
                                if pd.notna(emissions_value_kg):
                                    ...
                                    codecarbon_emissions_g = (
                                        codecarbon_emissions_kg * 1000.0
                                    )
                                    log_msg = (
                                        f"Read emissions: {codecarbon_emissions_kg:.6f}"
                                        f" kgCO2e ({codecarbon_emissions_g:.3f} gCO2e)"
                                        f" (attempt {attempt+1})"
                                    )
                                    logger.info(log_msg)
                                    break
                                    ...
                                    logger.warning(
                                        f"Read CSV attempt {attempt+1}, 'emissions' "
                                        "value NaN."
                                    )
                            elif not df.empty:
                                logger.warning(
                                ...
                                logger.warning(
                                    f"Read CSV attempt {attempt+1}, file empty."
                                )
                        except pd.errors.EmptyDataError:
                            logger.warning(
                                f"Read CSV attempt {attempt+1} failed: File empty."
                            )
                        except FileNotFoundError:
                            logger.error(
                                f"Read CSV attempt {attempt+1} failed: File "
                                f"disappeared {carbon_csv_path}."
                            )
                            break
                        except Exception as e_read:
                            logger.error(
                                f"Read CSV attempt {attempt+1} failed: {e_read}",
                                exc_info=True,
                            )
                        if attempt < max_retries - 1:
                            ...
                            logger.error(
                                "Failed to read emissions from "
                                f"{carbon_csv_path} after {max_retries} attempts."
                            )
                elif carbon_csv_path:
                    logger.warning(
                        f"CodeCarbon CSV path '{carbon_csv_path}' does not exist."
                    )
                else:
                    logger.warning("CodeCarbon CSV path not set.")
            except Exception as e_tracker:
                logger.error(
                    f"Error stopping/reading CodeCarbon tracker: {e_tracker}",
                    exc_info=True,
                )
        ...
        if total_energy_joules is not None:
            ...
            logger.info(
                f"Total GPU Energy (NVML Monitor): {total_energy_joules:.2f} J "
                f"({results['total_gpu_energy_wh']:.4f} Wh)"
            )
        elif monitor:
        ...
        if nvml_active and gpu_handle:
            ...
            if mem_info_end:
                logger.info(
                    f"GPU Mem (End): {mem_info_end[0]:.2f} MiB Used / "
                    f"{mem_info_end[1]:.2f} MiB Total"
                )
            else:
        ...
        final_summary_metrics = {
            ...
            "final/total_gpu_energy_wh": results.get(
                "total_gpu_energy_wh", float("nan")
            ),
            ...
            "final/estimated_fwd_gflops": results.get(
        ...
        if pd.notna(results.get("codecarbon_emissions_gCO2e")):
            logger.info(
                ...
                f"{results['codecarbon_emissions_gCO2e']:.3f} gCO2e"
            )
        else:
