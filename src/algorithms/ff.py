...
# --- Helper for Pixel Label Embedding (Copied from reference implementation) ---
def generate_ff_hinton_inputs(
    base_images: torch.Tensor,
    base_labels: torch.Tensor,
    num_classes: int,
    device: torch.device,
    replace_value_on: float = 1.0, # Hinton uses 1.0
    replace_value_off: float = 0.0, # Hinton uses 0.0
    neutral_value: float = 0.1, # Hinton uses 0.1 for neutral
    ...
    if num_classes > image_pixels:
        raise ValueError(
            f"num_classes ({num_classes}) > pixels per channel ({image_pixels}). Cannot embed label."
        )
...
# --- train_ff_model (NEW IMPLEMENTATION based on reference code logic) ---
def train_ff_model(
    ...
    input_adapter: Optional[Callable] = None, # Still ignored here
    step_ref: List[int] = [-1],
    ...
    except (ValueError, TypeError) as e: raise ValueError(f"Invalid LR/WD format: {e}") from e
    ...
    if ff_layer_params:
        ...
        optimizer_groups.append(group); logger.info(f"Opt Group 0 (FF): LR={ff_lr}, WD={ff_wd}" + (f", Mom={ff_momentum}" if 'momentum' in group else ""))
    if classifier_params:
        group = {"params": classifier_params, "lr": ds_lr, "weight_decay": ds_wd}
        if optimizer_type.lower() == 'sgd': group['momentum'] = ds_momentum
        optimizer_groups.append(group); logger.info(f"Opt Group 1 (DS): LR={ds_lr}, WD={ds_wd}" + (f", Mom={ds_momentum}" if 'momentum' in group else ""))
    if not optimizer_groups: logger.error("FF_MLP: No trainable parameters."); return 0.0

    try: optimizer_cls = getattr(optim, optimizer_type); optimizer = optimizer_cls(optimizer_groups); logger.info(f"Using optimizer: {optimizer_type}")
        ...
        for batch_idx, (images, labels) in enumerate(pbar):
            ...
            try:
                ...
                if not cls_loss.requires_grad and any(p.requires_grad for p in model.linear_classifier.parameters()):
                     cls_loss = cls_loss.clone().requires_grad_(True) # Clone to avoid modifying original if it came from no_grad context internally
            ...
            if torch.isnan(total_batch_loss) or torch.isinf(total_batch_loss): logger.error(f"NaN/Inf loss. Skipping."); continue
...
# --- evaluate_ff_model (NEW IMPLEMENTATION based on reference paper/code) ---
